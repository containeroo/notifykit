...
Application notification IDs are strings owned by the application. Queue IDs are Notifykit-owned UUIDv7 values. Receiver IDs remain strings. Failed enqueue operations return the zero UUID alongside the error.
...
manager, err := notify.NewManager(receivers, logger,
    ...
    notify.WithOnComplete(func(result notify.Completion) {
        // QueueID is a UUIDv7 and NotificationID is the application-owned string; nil Err means delivery succeeded.
