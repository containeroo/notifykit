...
`ID` returns a stable, non-empty application-owned string for logs and delivery tracing. Notifykit does not prescribe its format; synchronous send and queue admission only reject an empty ID.
